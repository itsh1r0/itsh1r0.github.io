...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const copyIdBtns = document.querySelectorAll('.copy-id-btn');
  ...
  const name = profileName.getAttribute('data-name') || "ITSUKA HIRO";
  let nameText = '';
  let nameIndex = 0;
  ...
  let bioMessages = ["Welcome to my profile!"];
  try {
    const bioData = profileBio.getAttribute('data-bio');
    if (bioData) {
      bioMessages = JSON.parse(bioData);
    }
  } catch (e) {
    console.error("Data bio error:", e);
  }

  const fullBioText = bioMessages.join('\n');
  let bioText = '';
  let bioIndex = 0;
  let bioCursorVisible = true;

  function typeWriterBio() {
    if (bioIndex < fullBioText.length) {
      bioText = fullBioText.slice(0, bioIndex + 1);
      bioIndex++;
  ...
  function copyInGameId(ingameId) {
    navigator.clipboard.writeText(ingameId).then(() => {
      showToast("Copied: " + ingameId);
    }).catch(err => {
  ...
  copyIdBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      copyInGameId(btn.getAttribute('data-ingame-id'));
    });
